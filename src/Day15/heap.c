...
/**
 * Modified from my repo: https://github.com/dcross23/Algorithms-and-Data-Structures
 */
...
void descFiltering(Heap *h, int posToFilter){
	if(h != NULL){
		...
		while(2*posToFilter<=h->size && !endFiltering){
			child=2*posToFilter;	
			if(child != h->size){
				if(h->elements[child+1].key  <  h->elements[child].key)
					child++;
			}
			
			if(h->elements[child].key  <  aux.key){
...
void ascFiltering(Heap *h, int posToFilter){
	if(h != NULL){
		heapElement aux = h->elements[posToFilter];
		while(posToFilter>1 && h->elements[posToFilter/2].key  >  aux.key){
...
int isHeap(Heap *h){
	...
	for(i=1; i<=n/2; i++){
		child = 2*i;
		// Checks left child
		if(h->elements[i].key  >  h->elements[child].key)
			return 0;

		// Checks right child
		else if(child != n && h->elements[i].key  >  h->elements[child+1].key)
			return 0;
	}
	return 1;
}
